...
const citizenId = params[params.length - 1];

$.ajax({
    url: "http://localhost:2323/api/citizens/" + citizenId,
    type: "GET",
    success: function (result) {
        if (result) {
            console.log("result", result);
            
            $("#name").text(result.citizen.name);
            $("#cmt").text(result.citizen.cmt);
            $("#dob").text(result.citizen.dob);
            $("#address").text(result.citizen.address);
            $("#job").text(result.citizen.job);
